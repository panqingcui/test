 ...
 * <p> To run this sample application, add the following jars to your classpath: <ul>
 * <li>archaius-core-xxx.jar (latest release/snapshot of archaius-core)
 * <li>commons-configuration-1.8.jar <li>commons-lang-2.6.jar
 * <li>commons-logging-1.1.1.jar </ul>
 * 
 * @author stonse
 * 
 */
public class SampleApplication extends Thread {
    ...
    // // You can use your own, or use of the pre built ones including
    ...
    // myConfiguration.setProperty("com.netflix.config.samples.sampleapp.prop1",
    // "value1");
 ...
 * Properties Some fields can get initialized at startup only and NOT change when actual
 * property value is changed. Others can use <code>DynamicProperty</code> and contain
 * values that are in sync with the runtime changes to the property source.
 * 
 * @author stonse
 * 
 */
class SampleBean {
    ...
    public SampleBean() {
        ...
        this.name = DynamicPropertyFactory.getInstance()
                .getStringProperty("com.netflix.config.samples.SampleApp.SampleBean.name", "Sample Bean").get();
    }
    ...
    public String name;
    /**
     * Number of seeds in this bean. The value for this is automatically populated by the
     * Configuration. Additionally, if the property value is modified, the new value will
     * be reflected as well We can assign a default value as well in case a value was not
     * configured or the configuration was not successfully loaded
     * 
     */
    public DynamicIntProperty numSeeds = DynamicPropertyFactory.getInstance().getIntProperty(
            "com.netflix.config.samples.SampleApp.SampleBean.numSeeds", 2);
    /**
     * This field utilizes the feature of Callbacks. When a property value is changed at
     * runtime, it will receive a callback which can be used to trigger other operations
     * or used for bookkeeping
     */
    DynamicStringProperty sensitiveBeanData = DynamicPropertyFactory.getInstance().getStringProperty(
            "com.netflix.config.samples.SampleApp.SampleBean.sensitiveBeanData", "magic", new Runnable() {
                public void run() {

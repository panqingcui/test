...
public class CreateBigImage {
    ...
    // for (int i = 0; i < len; i++) {
    // try {
    // images[i] = ImageIO.read(new ByteArrayInputStream(new
    // sun.misc.BASE64Decoder().decodeBuffer(str[i])));
    // } catch (Exception e) {
    // e.printStackTrace();
    // }
    ...
    // int height_i = 0;
    // for (int i = 0; i < images.length; i++) {
    // ImageNew.setRGB(0, height_i, dst_width, images[i].getHeight(), ImageArrays[i], 0,
    // dst_width);
    ...
    // * @return
    // */
    // public static String mergeFolderImgs(String folderPath, String imgType, String
    // outAbsolutePath) {
    ...
    // */
    // public static void changeImage(String path, String oldimg, String newimg, int
    ...
    // * @param newHeight 新图片高度
    // */
    // public static void changeFolderImages(String folderPath, int newWidth, int
    // newHeight) {
    // try {
    // File folder = new File(folderPath);// 得到文件夹
    // File[] imgList = folder.listFiles();// 得到文件夹中的所有图片
    // Image image = null;// 定义一张图片
    // BufferedImage bfImg = new BufferedImage(newWidth, newHeight,
    // BufferedImage.TYPE_INT_RGB);

...
// /**
// * 使用Sms 的RecvSms(int index)的方法时，使用该方法解析MODEM返回的字符串 根据MODEM返回的字符串，解析成一个CommonSms对象
...
// String datestring = s[len - 2].substring(0, s[len - 2].length() - 3).replace(',', ' ');// 短信时间格式09/09/09
// // 20:18:01+32
// Date date = null;
// try {
...
//
// /**
// * 使用Sms 的RecvSmsList()方法时，通过该方法解析MODEM返回来的字符串 根据MODEM返回的字符串，解析成一个CommonSms的集合对象
...
// cs.setState("未读");
// }
// cs.setSender((t[2].substring(t[2].indexOf('"') + 1, t[2].lastIndexOf('"')).trim()));
// SimpleDateFormat df = new SimpleDateFormat("yy/MM/dd hh:mm:ss");
// String datestring = t[4].substring(t[4].indexOf('"') + 1) + " " + t[5].substring(0, t[5].indexOf('"'));//
// 短信时间格式09/09/09
// // 20:18:01+32
// Date date = null;
// try {

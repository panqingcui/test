...
public class TestNeo4j {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            Relationship relationship = fromNode.createRelationshipTo(toNode, UserRelationship.FELLOW);
    ...
    public boolean saveGMap(GMap map) {
        ...
        try {
            ...
            if (viewport == null) {
                ...
                // 创建默认节点到根节点的关联
                graphDb.getReferenceNode().createRelationshipTo(viewport, TopoMapRelationshipTypes.ROOT);
            } else {
                ...
                for (int i = 0; i < map.getGLineList().size(); i++) {
                    ...
                    if (sn != null & en != null) {
                        if (sn.getProperty(PropertyKey.MAP_ID).equals(map.getId())
                            ...
                            this.createAndIndexLine(sn, en, TopoMapRelationshipTypes.CONNECTION, map.getId(), gline,
                                    lineIndex);
                        }
                    } else {
     ...
     * @return 节点对象
     */
    private Node createAndIndexNode(GraphDatabaseService graphDb, Index<Node> nodeIndex, GNode gn, String mapid) {
     ...
     * @return
     */
    private Relationship createAndIndexLine(Node sn, Node en, RelationshipType relType, String mapid, GLine gline,
    ...
    private Node getMapRootNode(String mapid) {
        ...
        Node root = null;
        // 搜索根节点
        Iterable<Relationship> rootIterable = graphDb.getReferenceNode().getRelationships(Direction.OUTGOING,
        ...
        for (Iterator<Relationship> iter = rootIterable.iterator(); iter.hasNext();) {
            ...
            if (node.getProperty(PropertyKey.MAP_ID) != null && node.getProperty(PropertyKey.MAP_ID).equals(mapid)) {
                root = node;
                break;
            }
        }
    ...
    public static void DeleteRelativeNode(Node n) {
        TraversalDescription td = Traversal.description().relationships(TopoMapRelationshipTypes.HAS, Direction.BOTH)

...
public class MongoDBManager {
    ...
    private static synchronized MongoDBManager getNewInstance(String dbName) {
        ...
        try {
            mm.mg = new Mongo(getProperty("mongodb.properties", dbName + ".host"), Integer.parseInt(getProperty(
                    "mongodb.properties", dbName + ".port")));
            mm.db = mm.mg.getDB(getProperty("mongodb.properties", dbName + ".dbname"));
        } catch (Exception e) {
     ...
     */
    public List<DBObject> findLess(String collection, DBObject q, DBObject fileds, DBObject orderBy, int pageNo,
            int perPageCount) {
        return getCollection(collection).find(q, fileds).sort(orderBy).skip((pageNo - 1) * perPageCount)
    ...
    public List<DBObject> find(String collection, DBObject q, DBObject orderBy, int pageNo, int perPageCount) {
        return getCollection(collection).find(q).sort(orderBy).skip((pageNo - 1) * perPageCount).limit(perPageCount)
                .toArray();
    }
     ...
     */
    public BasicDBList group(String collection, DBObject key, DBObject q, DBObject initial, String reduce,
            String finalize) {
     ...
     * @param orderBy 分组查询排序
     */
    public List<DBObject> mapReduce(String collection, String map, String reduce, String outputCollectionName,
            DBObject q, DBObject orderBy) {
    ...
    public List<DBObject> mapReduce(String collection, String map, String reduce, String outputCollectionName,
        ...
        return getCollection(outputCollectionName).find(null, new BasicDBObject("_id", false)).sort(orderBy)
